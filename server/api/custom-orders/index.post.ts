import { z } from 'zod'
import { addCustomOrder } from '~~/server/services/custom-order-service'
...
const schema = z.object({
  ...
  quoted_price: z.number().min(0, 'Harga tidak boleh negatif'),
...
export default defineEventHandler(async (event) => {
  ...
  const order = await addCustomOrder(event, data)
